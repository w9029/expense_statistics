...
- 验证: 待执行 `npx tsc --noEmit`、`npm run lint -- --quiet`、`npm test -- --runInBand --watchman=false`。
- 后续: 这轮趋势图交互增强确认后，再根据体验继续微调提示气泡和手势阈值。
...
- 提交: `fix(ios): lock analytics scroll during chart gesture`
- 范围: 分析页趋势图滚动锁定修复
- 说明: 为 `ScreenShell` 增加可控滚动开关，并在消费趋势折线图进入长按手势后临时关闭外层页面滚动，松手时再恢复；这样折线图内部长按和拖动不会再触发整页上下滚动，而图表外部仍保持正常滚动。
- 验证: 待执行 `npx tsc --noEmit`、`npm run lint -- --quiet`、`npm test -- --runInBand --watchman=false`。
- 后续: 如果你实测后仍遇到边缘情况，再继续收紧图表手势开始和结束时机。
...
- 提交: `chore(ios): make login the default public entry`
- 范围: 公开流入口改为登录页
- 说明: 将未登录用户的公开流默认入口从欢迎页切到登录页；同时把登录页底部改成两个独立入口按钮“找回密码 / 注册账号”，其中“找回密码”先作为正式入口位置落下，并通过提示说明当前 iOS 端尚未接入完整重置密码流程，避免继续把用户导向旧的欢迎页中转。
- 验证: 已执行 `npx tsc --noEmit`、`npm run lint -- --quiet`、`npm test -- --runInBand --watchman=false`。
- 后续: 如果后续确认要补全找回密码能力，可直接基于现有按钮入口继续接入邮件验证码与密码重置流程。

## 2026-05-11

- 提交: `fix(ios): require book name on list deletion`
- 范围: 账本列表页删除确认对齐
